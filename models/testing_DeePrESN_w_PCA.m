function error = testing_DeePrESN_w_PCA(parms)
...
try
    ...
    for j = 1:layers_res
            if j~=layers_res              
                    ...
                    [mappedX, mapping{j}] = compute_mapping(stateMatrix{j}, dim_reduc_name, floor(dim{j}));
    ...
    for j = 1:layers_res
            if j~=layers_res              
                    ...
                    mappedX = out_of_sample(stateMatrix{j}, mapping{j});

                    x_direct = input(nForgetPoints+1:end, :);
                    y_test =y_test(nForgetPoints+1:end,:);
    ...
    error{1} = error_measure( predicted_y(:,end), y_test(:,end), 'rmse');
    error{2} = error_measure( predicted_y(:,end), y_test(:,end), 'nrmse');
    error{3} = error_measure( predicted_y(:,end)+0.1, y_test(:,end)+0.1, 'mape');
